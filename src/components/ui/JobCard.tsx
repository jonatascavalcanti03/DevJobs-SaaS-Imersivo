...
export default function JobCard({ job, index = 0 }: JobCardProps) {
  ...
  const handleToggleSave = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/user/saved-jobs", {
        method: "POST",
        body: JSON.stringify({ jobId: job.id, isExternal }),
  ...
  const cardContent = (
    <motion.article
      ...
      } ${isExternal ? "opacity-90 hover:opacity-100" : ""}`}
    >
      {/* ── Hover Border Glow ── */}
      <div
        className={`absolute -inset-[1px] rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${
          job.isPremium
            ...
            : isExternal
            ? "bg-gradient-to-r from-[#64748B]/30 via-[#94A3B8]/10 to-[#64748B]/30"
      ...
      <div className={`relative glass-card rounded-2xl p-5 sm:p-6 ${isExternal ? "bg-[#080D1A]" : "bg-[#0B1121]"}`}>
        ...
        <div className="flex items-start gap-4">
          ...
          >
            {job.companyLogo ? (
              ...
              <Building2
                className={`w-6 h-6 ${
                  job.isPremium ? "text-[#FBBF24]" : isExternal ? "text-[#64748B]" : "text-[#818CF8]"
                }`}
              />
            )}
          </motion.div>
        ...
        <div className="flex items-center justify-between mt-5 pt-4 border-t border-white/5">
          ...
          <motion.span
            className={`flex items-center gap-1.5 text-xs font-medium opacity-0 group-hover:opacity-100 transition-all duration-300 translate-x-2 group-hover:translate-x-0 ${
              isExternal ? "text-[#64748B]" : "text-[#818CF8]"
            }`}
            whileHover={{ x: 3 }}
          >
            {isExternal ? "Ver no site parceiro" : "Ver vaga"}
  ...
  // Vagas externas abrem na nossa landing page interna primeiro
  if (isExternal) {
    return (
      <Link href={`/vagas/externa/${job.id}`} className="block w-full">
        {cardContent}
      </Link>
    );
  }

  // Vagas internas abrem na página interna padrão
  return (
    <Link href={`/vagas/${job.id}`} className="block w-full">
      {cardContent}
    </Link>
  );
}
