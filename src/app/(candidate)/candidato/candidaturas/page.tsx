  ...
  return (
    <div className="space-y-8 pb-20 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <motion.h1 initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="text-3xl sm:text-4xl font-extrabold text-text-primary mb-2 tracking-tight">
            Minhas Candidaturas
          </motion.h1>
          <motion.p initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="text-text-secondary text-lg">
        ...
        >
          <button className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-surface hover:bg-white/10 text-text-primary text-sm font-bold border border-border transition-colors">
      ...
      <div className="min-h-[400px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center glass-card rounded-3xl p-10">
            ...
            <h2 className="text-2xl font-bold text-text-primary mb-2">Nenhuma candidatura ainda</h2>
            <p className="text-text-secondary mb-6 max-w-md">Você ainda não se candidatou a nenhuma vaga. Explore nossa plataforma e encontre a oportunidade perfeita para você.</p>
            <Link href="/vagas" className="px-6 py-3 rounded-xl bg-gradient-to-r from-[#06B6D4] to-[#6366F1] text-text-primary font-bold hover:scale-105 transition-transform shadow-lg shadow-[#6366F1]/25">
            ...
            {candidaturas.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`relative h-full rounded-[23px] flex flex-col p-6 sm:p-8 ${isAccepted ? "bg-emerald-500/5 border border-emerald-500/20 shadow-sm shadow-emerald-500/5" : isRejected ? "bg-red-500/5 border border-red-500/20 shadow-sm shadow-red-500/5" : "bg-surface border border-border/50 shadow-sm transition-all hover:shadow-md"}`}>
                    ...
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="text-xl font-bold text-text-primary line-clamp-1">{item.job.title}</h3>
                    ...
                    <div className="mt-6 pt-6 border-t border-border/50 flex justify-end">
                      <Link href={`/vagas/${item.jobId}`}>
                        <button className="px-5 py-2.5 rounded-xl bg-surface hover:bg-white/10 text-text-primary text-sm font-bold transition-colors flex items-center gap-2">
