...
import { Search, Filter, Mail, Download, Star, Loader2, X, FileText, Globe, Code2, ExternalLink } from "lucide-react";
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
            ...
            >
              ...
              <div className="flex flex-col md:flex-row gap-6 items-start md:items-center mb-8">
                <div className="w-24 h-24 rounded-full border-4 border-[#06B6D4]/20 flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#06B6D4]/30 to-[#6366F1]/30 flex-shrink-0">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedProfile.skills ? (
                      selectedProfile.skills.split(',').map((skill: string, idx: number) => (
                        <span key={idx} className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[#E2E8F0] text-sm">
                          {skill.trim()}
                        </span>
                      ))
                    ) : (
                  ...
                  <div className="flex flex-col gap-3">
                    {selectedProfile.linkedin && (
                      <a href={selectedProfile.linkedin} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-[#94A3B8] hover:text-[#06B6D4] transition-colors bg-white/5 p-3 rounded-xl border border-white/5 w-fit">
                        <ExternalLink className="w-5 h-5" /> {selectedProfile.linkedin}
                      </a>
                    )}
                    {selectedProfile.portfolio && (
                      <a href={selectedProfile.portfolio} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-[#94A3B8] hover:text-[#06B6D4] transition-colors bg-white/5 p-3 rounded-xl border border-white/5 w-fit">
                        <Globe className="w-5 h-5" /> {selectedProfile.portfolio}
                      </a>
                    )}
                  ...
                  >
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full flex flex-shrink-0 items-center justify-center border border-white/10 overflow-hidden bg-gradient-to-br from-[#06B6D4]/30 to-[#6366F1]/30">
                    ...
                    <td className="px-6 py-5 text-center">
                      <div className="inline-flex items-center justify-center w-12 h-12 rounded-full border-4 border-emerald-500/20 relative">
                        <span className="text-xs font-bold text-emerald-400">{85 + (index % 15)}%</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          onClick={() => setSelectedProfile(candidato.user)}
